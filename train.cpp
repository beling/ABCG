...
void Train::reset() {
    for (std::list<Vicicle>::iterator i = wagons.begin(); i != wagons.end(); i++) {
        ...
        i->rightUp.prev_pos.x -= max_delta_r; */       //nadaje ��dan� pr�dko��
    }
...
void Train::satisfyLineConstraints(Link& link) {
    if (link.A.pos.x >= link.B.pos.x)   //belki odwr�cone nas nie interesuj�
        return;
    std::list<Vicicle>::iterator i = wagons.begin();
    //je�li lokomotywa jedzie po szynach, mo�na pr�bowa� przyspiesza�:
