...
void World::go() {
    ...
    for (std::list<Node>::iterator i = nodes.begin(); i != nodes.end(); i++) {
        i->goAndZeroForce(timeStep, enviroment(i->pos.y));        //poruszam w�z�y i zeruje si�y
	...
	train.satisfyConstraints();//Poprawia kszta�t wszystkich wagon�w
	for (std::list<Node*>::iterator i = unactive_nodes.begin(); i != unactive_nodes.end(); i++)
		(*i)->pos = (*i)->pos_0; //pilnujemy �eby w�z�y uwi�zane mia�y sta�� pozycje
	for (std::list<Link>::iterator i = links.begin(); i != links.end();)
		if (i->toLong()) {
			...
			i = links.erase(i);
			//TODO tu mo�na by pozby� si� z niczym nie po��czonych w�z��w
      } else ++i;         //usuwa zerwane �acza
}

void World::start() {
   	stop();
   	//obliczanie masy w�z��w:
    for (std::list<Link>::iterator i = links.begin(); i != links.end(); i++) {
        i->A.mass += i->mass / 2.0;
        i->B.mass += i->mass / 2.0;
    }
...
void World::stop() {
    time = 0.0;
	//kopiujemy link do g��wnej listy
    clone_links_list();

    for (std::list<Node>::iterator i = nodes.begin(); i != nodes.end(); i++)
   	    i->reset();  //resetuje w�z�y
...
std::list<Link>::iterator World::addLink(const double x0, const double y0, const double x1, const double y1, const double prec) {
    ...
    if (first == nodes.end())  //dodaje w�ze� je�li potrzebny
        first = addNode(Node(x0, y0, 500.0));

    if (second == nodes.end()) //dodaje w�ze� je�li potrzebny
        second = addNode(Node(x1, y1, 500.0));

    if (first->pos.x > second->pos.x) //sortujemy po wsp�rz�dnej x
...
void World::delAt(const double x, const double y, const double prec) {
    ...
    if (node == nodes.end()) {   //nie klikni�to na wie�cho�ek. TODO: poszuka� i usun�� link
        return;
    }
    //us�wamy wie�cho�ek
    ...
    for (std::list<Link>::iterator l = find_if(links_all.begin(), links_all.end(), std::bind2nd(Link::IsEnd(), *node));
         l != links_all.end(); l = find_if(l, links_all.end(), std::bind2nd(Link::IsEnd(), *node))) {
             ...
             l = links_all.erase(l); //us�wam link i przes�wam wska�nik
             if (count_if(links_all.begin(), links_all.end(), std::bind2nd(Link::IsEnd(), toDel)) == 0) {
                  unactive_nodes.remove(&toDel);
                  nodes.remove(toDel);        //jest nieu�ywany wi�c us�wamy
             }
         }
    clone_links_list();
